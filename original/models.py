...
class BwCombinedInteractModel50(nn.Module):
    ...
    def forward(self, full_image, black_image, white_image):
        ...
        f = self.full_image_model.maxpool(
            self.full_image_model.relu(
                self.full_image_model.bn1(
                    self.full_image_model.conv1(full_image))))
        b = self.black_image_model.maxpool(
            self.black_image_model.relu(
                self.black_image_model.bn1(
                    self.black_image_model.conv1(black_image))))
        w = self.white_image_model.maxpool(
            self.white_image_model.relu(
                self.white_image_model.bn1(
                    self.white_image_model.conv1(white_image))))
